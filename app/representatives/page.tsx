            ...
            <Link href={`/representatives/${rep.id}`} key={rep.id} className="block group">
              <div className="rep-card card-hover p-6 rounded-xl bg-bg-elevated border border-border-subtle h-full flex flex-col" style={{ opacity: 0 }}>
                ...
                <div className="space-y-3 text-sm text-text-secondary mb-6 flex-1">
                  <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-text-muted" /><span>{rep.constituency || 'N/A'}, {rep.district || 'Uttarakhand'}</span></div>
                  <div className="flex items-center gap-2"><Users className="w-4 h-4 text-text-muted" /><span>Attendance: <span className="text-success font-medium">{rep.attendance_rate ? rep.attendance_rate + '%' : 'N/A'}</span></span></div>
                </div>
